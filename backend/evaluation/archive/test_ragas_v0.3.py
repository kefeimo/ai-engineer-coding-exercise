...
print("=" * 60)
print("Test: RAGAS 0.3.x + GPT4All + LangchainLLMWrapper")
print("=" * 60)
print(f"RAGAS: 0.3.9")
...
start = time.time()
try:
    result = faithfulness.single_turn_score(sample)
    elapsed = time.time() - start
    print(f"\n{'=' * 60}")
    print(f"✓ SUCCESS! RAGAS 0.3.x + GPT4All works!")
    print(f"{'=' * 60}")
    print(f"Faithfulness Score: {result}")
    print(f"Evaluation Time: {elapsed:.2f}s")
    print(f"\n🎉 RAGAS 0.3.x supports GPT4All via LangchainLLMWrapper!")
    print(f"{'=' * 60}")
except Exception as e:
    elapsed = time.time() - start
    print(f"\n{'=' * 60}")
